...
/** Public read: upcoming dated sessions for a schedule with per-date availability (trial booking). */
export async function listTrialSessions(scheduleId, token) {
  return callFunction("list-trial-sessions", { schedule_id: scheduleId }, token);
}

/** Public write: book one free trial on a specific dated session. */
export async function bookTrial(body, token) {
  return callFunction("book-trial", body, token);
}
