...
test("schedule.js inserts a 'Classes' heading between the camps table and the weekly grid when camps exist", async () => {
  const script = await readSchedule();
  const match = script.match(/if \(bundles\.length > 0\) \{[\s\S]*?\r?\n {2}\}\r?\n\r?\n {2}\/\/ ---- Desktop weekly grid/);
...
test("renderCampsTable makes each camp row clickable to its enroll link, not just the Enroll button", async () => {
  const script = await readSchedule();
  const match = script.match(/function renderCampsTable\([\s\S]*?\r?\n}\r?\n/);
...
test("weekly schedule retains multiple classes sharing a day and start time", async () => {
  const script = await readSchedule();
  const match = script.match(/export function schedulesBySlot[\s\S]*?\r?\n}\r?\n/);
...
test("age group labels are prefixed with 'Age' when they are a bare digit range", async () => {
  const script = await readSchedule();
  const match = script.match(/export function schedulesBySlot[\s\S]*?\r?\n}\r?\n[\s\S]*?function formatAgeGroup[\s\S]*?\r?\n}\r?\n/);
...
test("parseSnapshot returns null for missing, empty, or malformed snapshot data", async () => {
  const script = await readSchedule();
  const match = script.match(/export function parseSnapshot[\s\S]*?\r?\n}\r?\n/);
...
test("pickDefaultSemester prefers Fall 2026, else falls back to the first semester", async () => {
  const script = await readSchedule();
  const match = script.match(/export function pickDefaultSemester[\s\S]*?\r?\n}\r?\n/);
...
test("changeSemester prefers the baked schedulesBySemester map before falling back to a live fetch", async () => {
  const script = await readSchedule();
  const fn = script.match(/async function changeSemester[\s\S]*?\r?\n}\r?\n/);
