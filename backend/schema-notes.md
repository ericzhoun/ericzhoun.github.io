...
## 2026-08-29 - standalone students, admin attendance and leave (migration env-gated, not applied)

- `students.user_id` becomes nullable (`backend/migrations/2026-08-29-students-nullable-user-id.sql`):
  the admin can record a student before the parent account exists. Existing rows are
  untouched. Apply BEFORE deploying the 2026-08-29 `admin-manage` changes.
- RLS impact: end-user policies compare `user_id` with `current_user_id()`, and equality
  never matches NULL, so standalone rows stay invisible to parents until a `user_id` is
  attached; service-key writes (admin-manage) are unaffected via the existing bypass.
- New `admin-manage` action `record-session-status` ({`enrollment_id`, `session_id`,
  `status` ∈ scheduled | attended | no_show | skipped}): records attendance, no-show,
  or an excused leave/请假 per enrollment and session. `skipped` preserves the credit
  (credits count attended bookings only, per the account-page formula). Creates a
  `home` booking on demand when none exists (comped/standalone enrollments often have
  none); re-recording is idempotent; cancelled make-ups stay historical.
- `add-student` / `create-enrollment` accept a missing `user_id` to create standalone
  rows. With a parent, the ownership check still applies.
- `admin-data` read surface: `enrollments` gains `enrollment_type` in reads and
  `student_id` / `schedule_id` / `status` filters (attendance sheet queries by
  schedule); new read-only `parent_profiles` resource for roster display.
